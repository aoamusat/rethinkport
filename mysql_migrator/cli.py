...
def build_config(args) -> Dict[str, Any]:
    ...
    config["mysql"]["database"] = os.environ.get("MYSQL_DATABASE", config["mysql"]["database"])
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    if not config.get("dry_run", False) and not mysql_config["password"]:
        print("Warning: No MySQL password provided. Set MYSQL_PASSWORD environment variable or use --password")
...
Examples:
  # Basic migration
  python -m rethinkdb_mysql_migrator /path/to/dump/
...
Environment Variables:
  ...
  MYSQL_DATABASE    MySQL database name
    ...
    parser.add_argument(
        "dump_path",
        help="Path to extracted RethinkDB dump directory"
    )
